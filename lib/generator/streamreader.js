...
const { AggregateBuffers } = require("./aggregatebuffers");
...
class StreamReaderReadable {
    ...
    constructor(stream, readStart, readEnd, options) {
        ...
        this.aggregateBuffers = new AggregateBuffers(this.queue, partSize);
        ...
        this.streamEndListener = () => this.onStreamEnd();
        ...
        this.stream.on("end", this.streamEndListener);
    ...
    read() {
        while (this.state === State.READ) {
            ...
            if (!chunk) {
                ...
                this.aggregateBuffers.push(chunk);
                if (this.queue.full) {
                    this.state = State.QUEUE_FULL;
                }
            }
