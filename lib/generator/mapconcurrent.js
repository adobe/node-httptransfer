...
async function waitResult(pending, ordered) {
    if (ordered) {
        ...
        if (i >= 0) {
            ...
            throw Error(`[internal] Promise ${promise} -> ${result} not in pending state`);
        }
    }
}
 ...
 */
async function executeBatch(mapFunction, values) {
    const result = [];
    for await (const x of mapFunction.execute(values)) {
        result.push(x);
    }
    return result;
}
    ...
    async* execute(items) {
        ...
        for await (const item of items) {
            if (checkAddBatch(this.mapFunction, batch, item, this.maxBatchLength)) {
                ...
                pending.push(executeBatch(this.mapFunction, batch));
                batch = [item];
            }
        }
