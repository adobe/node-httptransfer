 ...
 * @property {Number} [maxConcurrent=1] Maximum concurrent 
...
class MapConcurrent extends AsyncGeneratorFunction {
    /**
     * 
     * @param {AsyncGeneratorFunction} func Function to execute concurrently
    ...
    async* execute(items) {
        if (this.ordered) {
            // keep maxConcurrent invocations of this.func running
            // yield in fifo order
            const pending = [];
            for await (const value of items) {
                pending.push(this.func.execute(value));
            ...
            for await (const i of items) {
                pending.add(this.func.execute(i));        
