 ...
 * @param {CookieJar} cookieJar Cookie JAR
 * @param {Object} options Fetch options
 * @returns {*} received JSON
 */
async function postForm(url, form, cookieJar, options) {
    // pass in any retained cookies for the url
    ...
    // retain any set-cookies returned by server
    const cookies = response.headers.raw()[HTTP.HEADER.SET_COOKIE];
