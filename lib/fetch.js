...
const { HttpResponseError, HttpConnectError, HttpStreamError, HttpContentRangeError } = require("./error");
const { getETag, getContentLength, getLastModified, getContentType } = require("./headers");
...
function isTextContent(headers) {
    const contentType = headers.get(http.headers.CONTENT_TYPE);
    return contentType && (
 ...
 */
async function partialGet(url, offset, size, source, options) {
    // uncompressed range request
    // make sure that the block we are requesting is part of the same content
    const headers = {
        range: `${http.range.BYTES}=${offset}-${offset+size-1}`
    };
    if (source && source.etag) {
        headers[http.headers.IF_MATCH] = source.etag;
    } else if (source && source.lastModified) {
        headers[http.headers.IF_UNMODIFIED_SINCE] = source.lastModified;
    }
    ...
    // or 200 (OK) if range requests are not supported
    if (response.status === http.status.PARTIAL_CONTENT) {
        ...
        const contentRange = response.headers.get(http.headers.contentRange);
        if (!contentRange) {
            throw new HttpContentRangeError(http.method.GET, url, http.status, "missing content-type");
        }

        const parsed = parseContentRange(contentRange);
        if (parsed.unit !== http.range.BYTES) {
            throw new HttpContentRangeError(http.method.GET, url, http.status, `unsupported range type: ${contentRange}`);
        }
        ...
        const etag = getETag(response.headers);
        return {
            source: new Source(url, contentType, lastModified, etag, 0, size, contentLength),
            stream: response.body
        };
    } else {
