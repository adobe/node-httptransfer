 ...
 * @property {Readable} stream Readable stream
 */
/**
 ...
 */
function partialGetRequestHeaders(start, end, source) {
    const headers = {
        range: `${HTTP.RANGE.BYTES}=${start}-${end-1}`
    };
    if (source && source.etag) {
        headers[HTTP.HEADER.IF_MATCH] = source.etag;
    } else if (source && source.lastModified) {
        headers[HTTP.HEADER.IF_UNMODIFIED_SINCE] = source.lastModified;
    }
    return headers;
}

/**
 * Download partial content from the given URL
 * 
 * @param {String} url URL to download
 * @param {number} [start=0] Start offset of the block of bytes
 * @param {number} [end] End offset of the block of bytes, defaults to remainder of the content (exclusive)
 ...
 * @returns {*} HTTP response
 */
async function partialGet(url, start, end, previousSource, options) {
    return streamGet(url, Object.assign({
        compress: false,
        headers: partialGetRequestHeaders(start, end, previousSource)
    }, options));
}
