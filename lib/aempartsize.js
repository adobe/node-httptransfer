...
function calculatePartSize(numUrls, fileSize, minPartSize, maxPartSize, preferredPartSize) {
    if (numUrls <= 0) {
        throw new IllegalArgumentError("Invalid number of urls", numUrls);
    } else if (fileSize <= 0) {
        throw new IllegalArgumentError("Unsupported file size", fileSize);
    } else if (!isValidNumber(minPartSize) || (minPartSize <= 0)) {
        throw new IllegalArgumentError("Invalid minPartSize", minPartSize);
    } else if (!isValidNumber(maxPartSize) || (maxPartSize < minPartSize)) {
        throw new IllegalArgumentError(`Invalid maxPartSize (minPartSize=${minPartSize})`, maxPartSize);
    } else if (preferredPartSize && !isValidNumber(preferredPartSize)) {
        throw new IllegalArgumentError("Invalid preferredPartSize:", preferredPartSize);
    }
