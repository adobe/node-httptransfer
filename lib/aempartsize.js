...
/**
 * @typedef {Object} CalcPartSizeOptions
 * @property {Number} [minPartSize=1] Minimum part size
 ...
 * @param {CalcPartSizeOptions} [options] Part size options
 * @returns {Number} The calculated part size
 */
function calculatePartSize(numUrls, fileSize, maxPartSize, options) {
    const minPartSize = (options && options.minPartSize) || 1;
 ...
 * 
 * @param {URL[]|String[]} urls URLs for each part
 * @param {Number} fileSize Size of the file
 * @param {Number} partSize Size of the part
 */
function splitParts(urls, fileSize, partSize) {
    let low = 0, idx = 0;
    const result = [];
    while (low < fileSize) {
        const end = Math.min(low + partSize, fileSize);
        result.push({
            range: new DRange(low, end - 1),
            url: urls[idx]
        });
        low = end;
        ++idx;
