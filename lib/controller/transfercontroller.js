...
class TransferController extends EventEmitter {
    ...
    hasFailed(transferItem) {
        ...
        if (this.failedAssets.has(transferAsset)) {
            console.log('** TRANSFER CONTROLLER, transferItem has failed', transferAsset.target);
        } else {
            console.log('** TRANSFER CONTROLLER, transferItem has NOT failed', transferAsset.target);
        }
