...
class TransferController extends EventEmitter {
    ...
    notifyError(functionName, error, transferItem, props) {
        ...
        debug(`${failureEvents.length}th failure event recorded: ${error}`);
    ...
    async cleanupFailedTransfers() {
        for (const transferItem in this.failedAssets) {
            const { transferAsset } = splitTransferItem(transferItem);
            if (transferAsset.target.url.pathname) {
                ...
                try {
                    ...
                    debug(`Error ${JSON.stringify(error)} trying to unlink failed asset ${transferAsset.target.url.pathname}`);
                }
            }
        }
    }
    ...
    /**
     * Throw the first encountered error if any, otherwise do nothing
     */
    throwFirstError() {
        debug('############# failed assets errors');
        //console.log(this.failedAssets.entries);

        if (this.hasAnyFailed()) {
            // Subscripts translate to [0] first failed asset, [1] list of errors, [0] first error for first failed asset.
            
            throw this.failedAssets.entries[0][1][0];
        }
        debug('############# failed assets errors');
    }
}
