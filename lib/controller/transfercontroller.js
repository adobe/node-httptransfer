...
class TransferController extends EventEmitter {
    ...
    constructor() {
        ...
        this.transferringAssets = new Map();
    }
    ...
    /**
     * Informs the controller that the transfer for a given part has begun.
     *
     * @param {TransferPart} part Part that is now transferring.
     */
    transferringPart(part) {
        const { transferAsset } = part;
        if (!this.transferringAssets.has(transferAsset)) {
            this.transferringAssets.set(transferAsset, new Date().getTime());
        }
    }

    /**
     * Retrieves the timestamp when the controller was informed that the first
     * part of an asset began transferring. Will return 0 if no start time registered.
     *
     * @param {TransferAsset} transferAsset Asset whose transfer start time should be retrieved.
     */
    getTransferStartTime(transferAsset) {
        if (this.transferringAssets.has(transferAsset)) {
            return this.transferringAssets.get(transferAsset);
        }
        return 0;
    }
}
