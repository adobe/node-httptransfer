...
async function transferStream(sourceUrl, targetUrl, options) {
    ...
    if (!sourceRes.ok) {
        if (isTextContent(sourceRes.headers)) {
            ...
            throw Error(`Download from '${sourceUrl}' failed with status ${sourceRes.status}: ${message}`);
        } else {
            throw Error(`Download from '${sourceUrl}' failed with status ${sourceRes.status}`);
        }
    }
    ...
    if (!targetRes.ok) {
        if (isTextContent(sourceRes.headers)) {
            const message = await readTextStream(sourceRes.body, 10000);
            throw Error(`Upload to '${targetUrl}' failed with status ${targetRes.status}: ${message}`);
        } else {
            throw Error(`Upload to '${targetUrl}' failed with status ${targetRes.status}`);
        }
    }
