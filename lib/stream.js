...
const { HttpTransferError } = require('./error');
...
async function downloadStream(url, writeStream, options) {
    ...
    if (!response.ok) {
        if (isTextContent(response.headers)) {
            ...
            throw new HttpTransferError("GET", url, response.status, message);
        } else {
            throw new HttpTransferError("GET", url, response.status);
        }
    }

    return new Promise((resolve, reject) => {
        writeStream.on("error", err => {
            reject(new HttpTransferError("GET", url, response.status, err.message));           
        });
        response.body
            .pipe(writeStream)
...
async function uploadStream(readStream, url, options) {
    ...
    if (!response.ok) {
        if (isTextContent(response.headers)) {
            const message = await readTextStream(response.body, 10000);
            throw new HttpTransferError(fetchOptions.method, url, response.status, message);
        } else {
            throw new HttpTransferError(fetchOptions.method, url, response.status);
        }
    } else {
        // ignore successful response body
...
async function transferStream(sourceUrl, targetUrl, options) {
    const sourceRes = await fetch(sourceUrl, options && options.source);
    if (!sourceRes.ok) {
        if (isTextContent(sourceRes.headers)) {
            const message = await readTextStream(sourceRes.body, 10000);
            throw new HttpTransferError("GET", sourceUrl, sourceRes.status, message);
        } else {
            throw new HttpTransferError("GET", sourceUrl, sourceRes.status);
        }
    }
    ...
    const targetRes = await fetch(targetUrl, targetOptions)
    if (!targetRes.ok) {
        if (isTextContent(sourceRes.headers)) {
            const message = await readTextStream(sourceRes.body, 10000);
            throw new HttpTransferError(targetOptions.method, targetUrl, targetRes.status, message);
        } else {
            throw new HttpTransferError(targetOptions.method, targetUrl, targetRes.status);
        }
    }

    // ignore successful response body
    targetRes.body.destroy();
}
