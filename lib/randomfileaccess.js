...
class RandomFileAccess {
    ...
    /**
    * Read a section of a file using a stream
    * 
    * @param {String|URL} path Local path of the file to read from
    * @param {SubRange} range Range of bytes to read (limits included)
    * @returns {Stream} Read stream
    */
    getReadStream(path, range) {
        console.log(`Creating read stream for file ${path}, with range: low: ${range.low}, high: ${range.high}`);
    ...
    /**
     * Write a section to a file using a stream
     * 
     * @param {String|URL} path Path of the file to write to
     * @param {SubRange} range Range of bytes to write
     * @param {HttpResponse} httpResponse Stream of bytes to write
     * @param {Object} httpDetails Additional details about the http request (for nicer log and error messages)
     */
    writeHttpResponseStreamToFile(path, range, httpResponse, httpDetails) {
        console.log(`Creating write stream for file ${path}, with range: low: ${range.low}, high: ${range.high}`);
        ...
        return new Promise((resolve, reject) => {
            ...
            const stream = httpResponse.body;
            stream.once("error", (err) => {
                reject(new HttpStreamError(httpDetails.method, httpDetails.url, httpResponse.status, err));
            });
            ...
            stream.once("end", () => {
                if (writtenLength !== range.length) {
                    reject(new HttpStreamError(httpDetails.method, httpDetails.url, httpResponse.status, `Unexpected number of bytes read from stream. Expected ${range.length} but got ${writtenLength}.`));
                } else {
                    resolve();
                }
            });
