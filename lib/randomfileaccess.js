...
class RandomFileAccess {
     ...
     * @param {Number} chunkSize Length of bytes chunk to read
     */
    constructor() {
        this[PRIVATE] = {
            fileHandles: new Map()
    ...
    async read(path, range) {
        ...
        console.log('### TMN - Buffer allocation happens here:')
        console.log(`=========> Before allocation: Available memory (free): ${os.freemem()}`)
        // console.log(`=========> Before allocation: Total memory: ${os.totalmem()}`)

        let buffer;
        if(range.length < Buffer.poolSize){
            // use built-in nodejs buffer pool
            buffer = Buffer.allocUnsafe(range.length);
        } else {
            // find a smart way to deal with larger chunks
            buffer = Buffer.allocUnsafe(range.length);
        }
        console.log(Buffer);
        console.log(`Allocating ${range.length} for reading from file ${path}`);
        console.log(`=========> After allocation: Available memory (free): ${os.freemem()}`)
        // console.log(`=========> After allocation: Total memory: ${os.totalmem()}`)
