...
async function getOrOpen(fileHandles, path, flags, totalLength) {
    ...
    if (!fileHandlePromise) {
        console.log(`Opening file ${path} with flags ${flags} and total write length ${totalLength}`);
        fileHandlePromise = openExtendFile(path, flags, totalLength);
        fileHandles.set(path, fileHandlePromise);
    } else {
        console.log(`Reusing existing filehandle promise for file ${path} (requested flags ${flags} and total write length ${totalLength})`);
    }
