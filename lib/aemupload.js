...
const { AEMInitiateUpload, AEMInitiateUploadCheckAddBatch } = require("./functions/aeminitiateupload");
...
const { Batch } = require("./generator/batch");
const { Flatten } = require("./generator/flatten");
    ...
    async uploadFiles(options) {
        ...
        try {
            const pipeline = new Pipeline(
                new Batch(AEMInitiateUploadCheckAddBatch, { maxBatchLength: 100 }),
                new AEMInitiateUpload(),
                new Flatten(),
            ...
            for await (const x of pipeline.execute(
