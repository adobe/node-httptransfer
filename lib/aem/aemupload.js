...
class AEMUpload extends EventEmitter {
    ...
    async uploadFiles(options) {
        const isDirectBinaryEnabled = await AEMBinaryUpload.isDirectBinaryAccessEnabled(options);
        const binaryUpload = isDirectBinaryEnabled ? new AEMBinaryUpload(options) : new CreateAssetServletUpload(options);

        const controller = binaryUpload.createTransferController();
