...
function retryInit(options) {
    ...
    // note: retryBackoff currently not used in this lib!
    if (!options || (options.retryEnabled !== false)) {
        return {
            startTime: Date.now(),
            retryMaxDuration: (options && options.retryMaxDuration) || DEFAULT_MAX_MS_TO_TRY,
            retryInitialDelay: (options && options.retryInitialDelay) || DEFAULT_INITIAL_WAIT_MS,
            retryBackoff: (options && options.retryBackoff) || DEFAULT_BACKOFF,
