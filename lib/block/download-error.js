...
class DownloadError extends Error {
    ...
    static fromError(error, errorMessage = '') {
        ...
        if (message && code) {
            return new DownloadError(getFullMessage(errorMessage, message), code, stack);
        }

        if (message) {
            return new DownloadError(getFullMessage(errorMessage, message), errorCodes.UNKNOWN, stack);
        }

        if (typeof error === 'string') {
            return new DownloadError(getFullMessage(errorMessage, error), errorCodes.UNKNOWN);
        }

        try {
            return new DownloadError(getFullMessage(errorMessage, JSON.stringify(error)), errorCodes.UNKNOWN, stack);
        // eslint-disable-next-line no-unused-vars
        } catch (e) {
            return new DownloadError(getFullMessage(errorMessage, error), errorCodes.UNKNOWN, stack);
        }
    }
