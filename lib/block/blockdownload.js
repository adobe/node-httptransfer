...
async function* generateBlockDownloadTransfer(options) {
    ...
    console.log(`Generated ${assetCounter} transfer assets (files to download: ${expectedLength})`);
}
...
class BlockDownload extends EventEmitter {
    ...
    /**
     * Constructs a new block download (even emitter) instance
     */
    constructor(){
        super();
    ...
    createBlockDownloadController() {
        ...
        console.log("Block download: block download controller created");
        return controller;
    }
    ...
    async downloadFiles(options = {}) {
        ...
        console.log("Block download: transfer controller created");
        ...
        try {
            ...
            if (this.errorEvents && this.errorEvents.length > 0) {
                ...
                console.log("Errors encountered during block download");
            ...
            if (randomFileAccess) {
                console.log("Block download: closing random file accessor");
                await randomFileAccess.close();
                console.log("Block download: closed random file accessor");
            }
            if (controller) {
                console.log("Block download: cleaning up failed transfers");
