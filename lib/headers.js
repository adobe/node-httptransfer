...
function getLastModified(headers) {
    const lastModified = Date.parse(headers.get(http.headers.LAST_MODIFIED));
...
function getETag(headers) {
    const etag = headers.get(http.headers.ETAG);
...
function getContentDisposition(headers) {
    const value = headers.get(http.headers.CONTENT_DISPOSITION);
    if (value) {
        try {
...
function getContentType(headers) {
    const value = headers.get(http.headers.CONTENT_TYPE);
    if (value) {
        try {
...
function getSize(headers) {
    ...
    return (contentRange && contentRange.length) || contentLength;
}
