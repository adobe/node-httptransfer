...
class AEMDownload extends EventEmitter {
    ...
    async downloadFiles(options) {
        ...
        controller.on(TransferEvents.BEFORE, transferEvent => {
            if (transferEvent.functionName === "CreateTransferParts") {
                this.emit("filestart", transferEvent.transferAsset.eventData);
            } else if (transferEvent.functionName === "JoinTransferParts") {
        ...
        controller.on(TransferEvents.AFTER, transferEvent => {
            if (transferEvent.functionName === "JoinTransferParts") {
