 ...
 * @yields {{range, idx}} Tuple for each part
 */
function* generateParts(contentLength, partLength) {
    let low = 0;
    let idx = 0;
    while (low < contentLength) {
        const end = Math.min(low + partLength, contentLength);
        yield { range: new DRange(low, end - 1), idx };
        low = end;
        ++idx;
    }
}
...
class TransferPart {
    ...
    *split(targetUrls, minPartSize, maxPartSize, preferredPartSize) {
        if (this.acceptRanges) {
            ...
            for (const { range, idx } of generateParts(this.metadata.contentLength, partSize)) {
                yield new TransferPart(this, [targetUrls[idx]], range);
            }    
        } else {
