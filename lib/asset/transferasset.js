...
class TransferAsset {
    ...
    constructor(source, target, options) {
        ...
        if (options && options.nameConflictPolicy && !(options.nameConflictPolicy instanceof NameConflictPolicy)) {
            throw Error(`nameConflictPolicy is required to be of type NameConflictPolicy: ${options.nameConflictPolicy}`);
        }
        this[PRIVATE] = {
            source,
            target,
    ...
    set nameConflictPolicy(conflictPolicy) {
        if (!(conflictPolicy instanceof NameConflictPolicy)) {
            throw Error(`nameConflictPolicy is required to be of type NameConflictPolicy: ${conflictPolicy}`);
        }
