...
class TransferAsset {
    ...
    constructor(source, target, options) {
        if (!(source instanceof Asset)) {
            throw new IllegalArgumentError("source is required to be of type Asset", source);
        }
        if (!(target instanceof Asset)) {
            throw new IllegalArgumentError("target is required to be of type Asset", target);
        }
        if (options && options.metadata && !(options.metadata instanceof AssetMetadata)) {
            throw new IllegalArgumentError("metadata is required to be of type AssetMetadata", options.metadata);
        }
        if (options && options.version && !(options.version instanceof AssetVersion)) {
            throw new IllegalArgumentError("version is required to be of type AssetVersion", options.version);
        }
        if (options && options.multipartTarget && !(options.multipartTarget instanceof AssetMultipart)) {
            throw new IllegalArgumentError("multipartTarget is required to be of type AssetMultipart", options.multipartTarget);
        }
        if (options && options.nameConflictPolicy && !(options.nameConflictPolicy instanceof NameConflictPolicy)) {
            throw new IllegalArgumentError("nameConflictPolicy is required to be of type NameConflictPolicy", options.nameConflictPolicy);
        }
        this[PRIVATE] = {
            source,
            target,
    ...
    set metadata(metadata) {
        if (!(metadata instanceof AssetMetadata)) {
            throw new IllegalArgumentError("metadata is required to be of type AssetMetadata", metadata);
        }
    ...
    set version(version) {
        if (!(version instanceof AssetVersion)) {
            throw new IllegalArgumentError("version is required to be of type AssetVersion", version);
        }
    ...
    set multipartTarget(multipart) {
        if (!(multipart instanceof AssetMultipart)) {
            throw new IllegalArgumentError("multipartTarget is required to be of type AssetMultipart", multipart);
        }
    ...
    set nameConflictPolicy(conflictPolicy) {
        if (!(conflictPolicy instanceof NameConflictPolicy)) {
            throw new IllegalArgumentError("nameConflictPolicy is required to be of type NameConflictPolicy", conflictPolicy);
        }
