...
const MAX_PERCENTAGE_ALLOWED_MEMORY_USE = 0.60;
...
class TransferMemoryBlock {
     ...
     * 
     * Usable also for functions needing to use a buffer instead of a TransferMemoryBlock
     * 
     * @returns {Buffer} Buffer with data from a piece of memory
     */
    get buffer() {
    ...
    /**
     * Get the length of the allocated buffer representing 
     * the memory block from the pooled buffer allocated for this Transfer Memory Block.
     * @returns {Integer} Allocated buffer length
     */
    get bufferLength() {
        if (this[INTERNAL].referencedMemoryFromBufferPool) {
            return this[INTERNAL].referencedMemoryFromBufferPool.length;
        }
        return 0;
    }
}
...
class TransferMemoryBuffer {
     ...
     * allocated instead
     */
    constructor(suggestedSize, allowDeferredAllocations = "true") {
        this[PRIVATE] = {};
        ...
        if (!bufferSize || !Number.isInteger(bufferSize)) {
            throw new Error("Suggested Transfer Memory Buffer total suggested size must be an integer");
        }
        if (bufferSize <= 0) {
            throw new Error("Transfer Memory Buffer total suggested size must be larger than 0");
        }

        this[PRIVATE].allowDeferredAllocations = process.env.NODE_HTTPTRANSFER_ALLOW_ALLOCATIONS_TO_WAIT || allowDeferredAllocations;
        console.log(`Allow deferred buffer allocations for transfers: ${this[PRIVATE].allowDeferredAllocations}`);

        let availableTotalMemory;
        try {
            console.log(cgroup.memory)
            console.log(cgroup.memory)
            availableTotalMemory = cgroup.memory.containerMemoryLimit();
        } catch (err) {
            // cgroup could not be read
            console.log(err);
            availableTotalMemory = os.freemem();
        }
        console.log("Available total memory in bytes is: ", availableTotalMemory);
        const highMemoryMark = Math.floor(availableTotalMemory * MAX_PERCENTAGE_ALLOWED_MEMORY_USE);
        console.log("High memory mark is ", highMemoryMark);
        if (bufferSize > highMemoryMark) {
            // fail instead, so we can remove the pending allocations 
            this[PRIVATE].totalSize = Math.min(highMemoryMark, MAX_BUFFER_SIZE);
            console.log(`Not enough memory available (${availableTotalMemory} bytes usable, ${bufferSize} bytes requested) to use suggested size for Transfer Memory Buffer pool size. Using ${MAX_PERCENTAGE_ALLOWED_MEMORY_USE * 100}% of available memory instead: ${this[PRIVATE].totalSize} bytes as Transfer Memory Buffer pool size`);
        } else {
        ...
        if (this[PRIVATE].allowDeferredAllocations === "true") {
    ...
    /**
     * Returns a list of the currently allocated (memory) blocks from the pooled buffer.
     * Those memory blocks are slices of the pooled buffer
     */
    get allocatedBlocks() {
    ...
    async obtainBuffer(bufferSize) {
        ...
        if (bufferSize > this[PRIVATE].availablePooledBufferSize) {
            if (this[PRIVATE].allowDeferredAllocations === "true") {
                ...
                throw new Error("Not enough memory available for transfer");
            }
        } else {
    ...
    maybeAllocateFromPendingAllocations() {
        if (this[PRIVATE].allowDeferredAllocations === "false" || this[PRIVATE].pendingAllocations.length <= 0) {
            return;
        }
     ...
     * 
     * @returns JSON representation of deferred allocations currently
     * waiting for memory
     */
