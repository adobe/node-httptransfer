...
class TransferMemoryBlock {
    ...
    constructor(size, startIndex, referencedMemoryFromBufferPool) {
        ...
        if (size !== referencedMemoryFromBufferPool.length) {
...
class TransferMemoryBuffer {
    ...
    releaseBuffer(allocatedBufferBlock) {
        if (!allocatedBufferBlock
            ...
            console.log("No buffer memory block to release");
            return;
        }
        ...
        if (this[PRIVATE].allocatedBlocks.length === 1) {
            ...
            console.log("All allocated buffer memory blocks have been released");
        } else {
        ...
        if (blockToRelease) {
            this[PRIVATE].availablePooledBufferSize += blockToRelease.value.size;
        }
    }
}
