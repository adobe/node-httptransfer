...
class TransferMemoryBuffer {
    ...
    constructor(suggestedSize) {
        ...
        console.log(`############ TMN - Requesting buffer pool of size ${suggestedSize}`);
        this[PRIVATE] = {};
    ...
    async obtainBuffer(bufferSize) {
        if (bufferSize > this[PRIVATE].totalSize) {
            throw new Error(`SEGFAULT - Requested ${bufferSize} bytes for transfer part, which is larger than max available memory of ${this[PRIVATE].totalSize} bytes for transfers`);
        }
        ...
        console.log(`############ TMN - Obtaining buffer block of size ${bufferSize}`);
        let bufferBlock;
        console.log("this[PRIVATE].availablePooledBufferSize: ", this[PRIVATE].availablePooledBufferSize)
    ...
    findNextAvailableBufferBlock(bufferSize) {
        ...
        if (this[PRIVATE].allocatedBlocks.length === 0) { // TMN - TODO Refactor to remove this
            ...
            while (foundBufferBlock === null && currentNode !== null) {
                ...
                if (nextNode === null) {
                    ...
                    if (candidateBufferBlockIndex > this[PRIVATE].maxIndex) {
                        console.log('### TMN - New start node')
    ...
    maybeAllocateFromPendingAllocations() {
        console.log('### TMN - maybeAllocateFromPendingAllocations')
