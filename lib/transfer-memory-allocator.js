...
const DEFAULT_TRANSFER_BUFFER_POOL_SIZE = 100000000; // 100 Mb
...
class DeferredTransferMemoryBlockAllocation {
    constructor(requestedMemoryBufferBlockSize) {
        ...
        // this[ALLOCATION_PROMISE_INTERNAL].creationTime = Date.now();
    }
    ...
    async promiseTransferMemoryBlockAllocation() {
        const self = this;
        this[ALLOCATION_PROMISE_INTERNAL].allocationPromise = new Promise(function (resolve, reject) {
            self.resolve = resolve;
            self.reject = reject;
        });

        return this[ALLOCATION_PROMISE_INTERNAL].allocationPromise;
    }
...
class TransferMemoryBuffer {
    ...
    constructor(suggestedSize) {
        ...
        if (bufferSize > highMemoryMark) {
            ...
            this[PRIVATE].totalSize = highMemoryMark;
    ...
    async obtainBuffer(bufferSize) {
        console.log("### TMN - Obtaining buffer of size: ", bufferSize)
        ...
        let bufferBlock;
        console.log(`Available pool buffer size is ${this[PRIVATE].availablePooledBufferSize}`)
        console.log(this.dumpBufferBlockUsedMemory())
        ...
        if (bufferBlock && bufferBlock.size && Number.isInteger(bufferBlock.size)) {
            console.log(`### TMN obtain - Updating availablePooledBufferSize from ${this[PRIVATE].availablePooledBufferSize} to ${this[PRIVATE].availablePooledBufferSize - bufferBlock.size}`)
    ...
    attemptAllocatingFromPendingAllocations() {
        ...
        while (nextAllocation === null && possibleNextAllocation !== null) {
            ...
            if (allocationRequest.requestedSize <= this[PRIVATE].availablePooledBufferSize) {
                ...
                if (bufferMemoryBlock) { // could allocate, cans top going through list
                    nextAllocation = possibleNextAllocation;
                } // no allocation possible: needs to continue looking if any pending allocation can be evacuated
            } else {
                possibleNextAllocation = possibleNextAllocation.next;
            }
        }

        if (nextAllocation && bufferMemoryBlock) {
            const resolvedAllocation = nextAllocation.value;
            this[PRIVATE].pendingAllocations.removeNode(nextAllocation);

            console.log(`### TMN obtaining after release - Updating availablePooledBufferSize from ${this[PRIVATE].availablePooledBufferSize} to ${this[PRIVATE].availablePooledBufferSize - bufferMemoryBlock.size}`)
    ...
    releaseBuffer(allocatedBufferBlock) {
        ...
        if (blockToRelease) {
            ...
            if (releasedSize <= 0) {
                ...
                console.log(`### TMN release - Updating availablePooledBufferSize from ${this[PRIVATE].availablePooledBufferSize} to ${this[PRIVATE].availablePooledBufferSize + releasedSize}`)
                this[PRIVATE].availablePooledBufferSize += releasedSize;
                this.attemptAllocatingFromPendingAllocations();
            }
        }
    }
}
