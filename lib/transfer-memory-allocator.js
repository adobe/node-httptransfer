...
class TransferMemoryBlock {
    ...
    constructor(size, startIndex, referencedMemoryFromBufferPool) {
        ...
        if (referencedMemoryFromBufferPool && size !== referencedMemoryFromBufferPool.length) {
            throw new Error("Requested buffer memory block size mismatch. Could not allocate Transfer Memory Block");
        }
...
class TransferMemoryBuffer {
    ...
    async obtainBuffer(bufferSize) {
        ...
        if (bufferSize > this[PRIVATE].availablePooledBufferSize) {
            console.log(`### TMN - Not enough memory available currently to allocate buffer block of size ${bufferSize} bytes (${this[PRIVATE].availablePooledBufferSize} bytes available. Creating promise to wait for available memory)`)
    ...
    findNextAvailableBufferBlock(bufferSize) {
        console.log(`### TMN - Looking for a memory block of size ${bufferSize}`)
        console.log(this[PRIVATE].allocatedBlocks)
            ...
            if (releasedSize <= 0) {
                ...
                while (nextAllocation === null && possibleNextAllocation !== null) {
                    ...
                    if (allocationRequest.requestedSize <= this[PRIVATE].availablePooledBufferSize) {
                        console.log("Hit next possible allocation!")
