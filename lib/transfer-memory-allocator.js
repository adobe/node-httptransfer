    ...
    obtainBuffer(bufferSize) {
        ...
        if (this[PRIVATE].allocatedBlocks.length === 0) {
            ...
            if (bufferSize > this[PRIVATE].availablePooledBufferSize) {
                console.log(`### TMN - Not enough memory available currently to allocate buffer block of size ${bufferSize} bytes (${this[PRIVATE].availablePooledBufferSize} bytes available)`);
            } else {
