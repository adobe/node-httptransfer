...
const NODE_HTTPTRANSFER_ALLOW_ALLOCATIONS_TO_WAIT = process.env.NODE_HTTPTRANSFER_ALLOW_ALLOCATIONS_TO_WAIT || false;
...
class TransferMemoryBuffer {
    ...
    constructor(suggestedSize) {
        ...
        if (NODE_HTTPTRANSFER_ALLOW_ALLOCATIONS_TO_WAIT) {
    ...
    maybeAllocateFromPendingAllocations() {
        if (!NODE_HTTPTRANSFER_ALLOW_ALLOCATIONS_TO_WAIT || this[PRIVATE].pendingAllocations.length <= 0) {
            return;
        }
