...
const DEFAULT_BLOCK_SIZE = 20*1024*1024;
...
async function openGetStream(url, start, end, previousSource, options) {
    ...
    if (response.status === HTTP.STATUS.PARTIAL_CONTENT) {
        ...
        sourceEnd = contentRange.last + 1; // http range is inclusive, our range is exclusive hence the + 1
...
async function downloadFile(url, filepath, options) {
    ...
    const { source, stream }  = await retry(async () => {
    ...
    const transferQueue = map_concurrent_unordered(
        ...
        async ({ source, buffer }) => {
            // console.log(targetUri, source.contentType, source.start, source.end);
            ...
            if (!buffer) {
                buffer = await retry(async () => {
                    ...
                    const block = await readBlock(source, source.start, source.end, partialStream, partialSource.start);
    ...
    for await (const block of transferQueue) {
        ...
        console.log(`Transferred ${transferKB} KBytes in ${elapsed} seconds at ${transferKB/elapsed} KB/s`);
