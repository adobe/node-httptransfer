...
class Transfer extends AsyncGeneratorFunction {
    ...
    async* execute(transferParts, controller) {
        for await (const transferPart of transferParts) {
            try {
                controller.notifyBefore(this.name, transferPart);
                await transfer(transferPart, this.randomFileAccess, this.options);
                controller.notifyAfter(this.name, transferPart);
                yield transferPart;
                controller.notifyYield(this.name, transferPart);
            } catch (error) {
                controller.notifyFailure(this.name, error, transferPart);
            }
        }
    }
}
