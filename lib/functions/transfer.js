...
async function transfer(transferPart, randomFileAccess, options) {
    ...
    if (isFileProtocol(transferPart.source.url) && targetUrl) {
        const buf = await randomFileAccess.read(transferPart.source.url, contentRange);
        await retry(async () => {
        ...
        await retry(async () => {
            ...
            await randomFileAccess.write(targetUrl, contentRange, buffer, totalSize);
        });
    }
}
...
class Transfer extends AsyncGeneratorFunction {
    ...
    async* execute(transferParts, controller) {
        for await (const transferPart of transferParts) {
            try {
                controller.notifyBefore(this.name, transferPart.transferAsset, transferPart);
                await transfer(transferPart, this.randomFileAccess, this.options);
                controller.notifyAfter(this.name, transferPart.transferAsset, transferPart);
                yield transferPart;
                controller.notifyYield(this.name, transferPart.transferAsset, transferPart);
            } catch (error) {
                controller.notifyFailure(this.name, error, transferPart.transferAsset, transferPart);
            }
        }
    }
}
