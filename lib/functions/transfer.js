    ...
    async* execute(transferParts, controller) {
        for await (const transferPart of transferParts) {
            try {
                ...
                console.log(`Transferred content range: ${JSON.stringify(contentRange)}`);
                if (process.env.USE_STREAM && isFileProtocol(transferPart.source.url) && targetUrl) {
                    // uses streams instead of buffers
                    console.log("Source has protocol 'file', and using streams is set (upload)");
                    const stream = await this.randomFileAccess.getReadStream(transferPart.source.url, contentRange);
                    ...
                    await retry(async () => {
                        const requestOptions = {
                            ...
                            headers: Object.assign({
                                [HTTP.HEADER.CONTENT_LENGTH]: stream.end - stream.start +1,
                        ...
                        console.log(`#### Issuing PUT to ${targetUrl}`);
                        await issuePut(targetUrl, requestOptions);
                    }, this.options);
                } else if (isFileProtocol(transferPart.source.url) && targetUrl) {
                    ...
                    console.log("Target has protocol 'file' (download)");
                    ...
                    await retry(async () => {
                        ...
                        const httpDetails = {
                            ...
                            url: transferPart.source.url
                        };
                        await this.randomFileAccess.writeHttpResponseStreamToFile(targetUrl, contentRange, response, httpDetails);
                    }, this.options);
                }
