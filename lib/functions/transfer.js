        ...
        for await (const transferPart of transferParts) {
            try {
                ...
                if (process.env.USE_STREAM && isFileProtocol(transferPart.source.url) && targetUrl) {
                    ...
                    console.log("#### Transfer is file protocol (using streams)");
                    ...
                    console.log("#### Transfer is file protocol (using buffers)");
                    const buf = await this.randomFileAccess.read(transferPart.source.url, contentRange);

                    console.log(`#### Used buffer length (determines header range): ${buf.length}`);
                    await retry(async () => {
                        const requestOptions = {
                            body: buf,
                            timeout: this.options && this.options.timeout,
                    ...
                    console.log("#### Transfer is blob transfer");
                    ...
                    console.log("#### Transfer target is file protocol");
                    await retry(async () => {
