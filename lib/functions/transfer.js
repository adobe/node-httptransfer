...
const { isFileProtocol, streamToPooledBuffer } = require("../util");
    ...
    async* execute(transferParts, controller) {
        for await (const transferPart of transferParts) {
            ...
            try {
                ...
                logger.info(`Transferred content range: ${JSON.stringify(contentRange)}`);
                if (isFileProtocol(transferPart.source.url) && targetUrl) {
                    const bufferMemoryBlock = await this.randomFileAccess.read(transferPart.source.url, contentRange);

                    await retry(async () => {
                        const requestOptions = {
                            body: bufferMemoryBlock.buffer,
                            timeout: this.options && this.options.timeout,
                            headers: Object.assign({
                                [HTTP.HEADER.CONTENT_LENGTH]: bufferMemoryBlock.bufferLength,
                    ...
                    this.randomFileAccess.releaseFileBufferMemory(bufferMemoryBlock);
                    ...
                    await retry(async () => {
                        ...
                        const bufferMemoryBlock = await streamToPooledBuffer(HTTP.METHOD.GET, transferPart.source.url, response.status, response.body, contentLength, this.randomFileAccess.bufferMemoryAllocator);

                        await this.randomFileAccess.write(targetUrl, contentRange, bufferMemoryBlock, totalSize);
                        this.randomFileAccess.releaseFileBufferMemory(bufferMemoryBlock);
                    }, this.options);
                }
