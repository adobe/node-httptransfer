        ...
        for await (const transferPart of transferParts) {
            try {
                ...
                if (isFileProtocol(transferPart.source.url) && targetUrl) {
                    console.log("#### Transfer is file protocol")
                    const stream = this.randomFileAccess.readStream(transferPart.source.url, contentRange);
                    await retry(async () => {
                        const requestOptions = {
                            body: stream,
                            timeout: this.options && this.options.timeout,
                            headers: Object.assign({
                                [HTTP.HEADER.CONTENT_LENGTH]: stream.end+1,
