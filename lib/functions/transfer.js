    ...
    async* execute(transferParts) {
        for await (const transferPart of transferParts) {
            ...
            if (contentRanges.length !== 1) {
                throw new IllegalArgumentError("Only support a single range at this point", contentRanges);
            }
            const contentRange = contentRanges[0];
            if (contentRange.length > MAX_MEMORY_BUFFER) {
                throw new IllegalArgumentError("Content range too large, not supported yet", transferPart.source.url);
            }
