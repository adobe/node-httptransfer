    ...
    async* execute(transferParts) {
        for await (const transferPart of transferParts) {
            ...
            if (contentRanges.length !== 1) {
                throw new IllegalArgumentError("'contentRanges' must be a single range", contentRanges);
            }
            const contentRange = contentRanges[0];
            if (contentRange.length > MAX_MEMORY_BUFFER) {
                throw new IllegalArgumentError("'contentRange.length' too large, not supported yet", transferPart.source.url);
            }
