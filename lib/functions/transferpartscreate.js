        ...
        for await (const transferAsset of transferAssets) {
            try {
                ...
                if (transferAsset.acceptRanges && transferAsset.multipartTarget) {
                    ...
                    debug(`${idx} transfer parts created (multipart target)`);  
                    ...
                    debug(`${idx} transfer parts created (target uses file protocol)`);
                } else {
                    const targetUrls = [transferAsset.target.url];
                ...
                debug(`Error during transfer parts creation: ${JSON.stringify(error)}`);
