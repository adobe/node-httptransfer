            ...
            try {
                ...
                if (transferAsset.acceptRanges && transferAsset.multipartTarget) {
                    ...
                    for (const range of generatePartRanges(contentLength, partSize)) {
                        const transferPart = controller.createTransferPart(transferAsset, [targetUrls[idx]], range, targetHeaders);
                    ...
                    let idx = 0;
                    for (const range of generatePartRanges(contentLength, partSize)) {
                        const transferPart = controller.createTransferPart(transferAsset, targetUrls, range, transferAsset.target.headers);
                        controller.notify(TransferEvents.AFTER_CREATE_TRANSFER_PARTS, this.name, transferPart);
                        yield transferPart;
                        ++idx;
                    }
                    ...
                    const transferPart = controller.createTransferPart(transferAsset, targetUrls, contentRange, transferAsset.target.headers);
