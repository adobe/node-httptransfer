...
class AEMInitiateUpload extends AsyncGeneratorFunction {
     ...
     * 
     * @generator
     * @param {TransferPart[]|Generator||AsyncGenerator} transferRecords Transfer records, target going to AEM
    ...
    async* execute(transferRecords) {
        ...
        for (let i = 0; i < records.length; ++i) {
            ...
            record.completeUrl = resolvedCompleteUrl;
            record.uploadToken = uploadToken;            
            yield* record.split(uploadURIs, minPartSize, maxPartSize, preferredPartSize);
        }
    }
}
