...
class AEMInitiateUpload extends AsyncGeneratorFunction {
    ...
    async* execute(transferAssetsGen, controller) {
        //TODO: This needs some rework, explicitly batching might be better
        ...
        try {
            ...
            // submit batch
            const folderUrl = transferAssets[0].target.folderUrl;
            const initiateResponse = await retry(async () => {
                return postForm(`${folderUrl}.initiateUpload.json`, form, {
                    ...
                    headers: transferAssets[0].target.headers
                });
            }, this.options);
            ...
            } else if (!Array.isArray(initiateResponse.files) || (initiateResponse.files.length !== transferAssets.length)) {
                throw Error(`'files' field incomplete in initiateUpload response (expected files: ${transferAssets.length}): ${JSON.stringify(initiateResponse)}`);
