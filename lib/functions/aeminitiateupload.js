...
class AEMInitiateUpload extends AsyncGeneratorFunction {
    ...
    async* execute(transferAssets) {
        ...
        if (!Array.isArray(initiateResponse.files)) {
            ...
            throw Error(`Number of files returned from initiateUpload does not match number of assets sent. Requested: ${assets.length}, Received: ${initiateResponse.files.length}`);
