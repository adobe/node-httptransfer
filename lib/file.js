...
async function downloadFile(url, filepath, options) {
    return retry(async options => {
        ...
        console.log('WRITESTREAM SIZE', size);
        console.log('TRANSFERRED SIZE', transferred);
        console.log('WRITESTREAM FINISHED:', writeStream.writableFinished);
        console.log('WRITESTREAM LENGTH', writeStream.writableLength);
        console.log('WRITESTREAM NEEDS DRAIN', writeStream.writableNeedDrain);
        if (size !== transferred) {
            const sleep = require('util').promisify(setTimeout);
            await sleep(5000);
            console.log('WRITE STREAM SIZE 5 SECONDS LATER', (await fs.stat(filepath)).size);
            await sleep(10000);
            console.log('WRITE STREAM SIZE 10 SECONDS LATER', (await fs.stat(filepath)).size);
