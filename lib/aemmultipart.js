...
async function uploadAEMMultipartFile(filepath, target, options) {
    ...
    const transferPart = new TransferPart(
        new Asset(pathToFileURL(filepath)), 
        new Asset(target.urls[0], undefined, options && options.headers)
    );
    const { size } = await fs.stat(filepath);
    const contentType = (options && options.contentType) || mime.lookup(filepath);
    transferPart.metadata = new AssetMetadata(filepath, contentType, size);
    transferPart.acceptRanges = true;

    // Execute pipeline
    // Default the minimum part size to the max (favor least amount of urls)
    const minPartSize = target.minPartSize || target.maxPartSize;
    const preferredPartSize = options && options.partSize;
    await pipeline.execute(
        transferPart.split(target.urls, minPartSize, target.maxPartSize, preferredPartSize)
    );
}
